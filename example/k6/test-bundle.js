...
`);
    await test("createUser roundtrip", async () => {
      ...
      try {
        ...
        assert2(resp.user !== void 0, "user should be defined");
      ...
      try {
        ...
        try {
          ...
          assert2(
            ...
            `expected RemoteError, got ${e.constructor.name}`
          );
          assert2(
    ...
    await test("raw.echo \u2014 binary roundtrip", async () => {
      ...
      try {
        ...
        const payload = new Uint8Array([222, 173, 190, 239, 0, 128, 255]);
    ...
    await test("raw.download \u2014 server streams binary chunks", async () => {
      ...
      try {
        ...
        await ws.send(new Uint8Array([3]));
        for (let i = 0; i < 3; i++) {
          const chunk = await ws.recv();
          assertEqual(chunk.length, 64, `chunk[${i}] length`);
          assertEqual(chunk[0], i * 64 & 255, `chunk[${i}] first byte`);
        }
        ws.close();
      } finally {
        session.close();
    ...
    await test("raw.transform \u2014 bidi XOR", async () => {
      ...
      try {
        ...
        const input = new Uint8Array([0, 85, 170, 255]);
        await ws.send(input);
        const result = await ws.recv();
        assertEqual(result.length, 4, "result length");
        assertEqual(result[0], 255, "byte[0]");
        assertEqual(result[1], 170, "byte[1]");
        assertEqual(result[2], 85, "byte[2]");
        assertEqual(result[3], 0, "byte[3]");
        ws.close();
      } finally {
        session.close();
      }
