(() => {
  ...
  // ../../ts/dist/src/yamux.js
  var VERSION = 0;
  var TYPE_DATA = 0;
  var TYPE_WINDOW_UPDATE = 1;
  var TYPE_PING = 2;
  var TYPE_GO_AWAY = 3;
  var FLAG_SYN = 1;
  var FLAG_ACK = 2;
  ...
  var EOF = class extends Error {
    constructor() {
      super("EOF");
      this.name = "EOF";
    }
  };
  var YamuxSession = class _YamuxSession {
    ws;
    streams = /* @__PURE__ */ new Map();
    nextID = 1;
    // Client uses odd stream IDs
    sessionBuffer = new ByteBuffer();
    closed = false;
    constructor(ws) {
      this.ws = ws;
    }
    ...
    static async connect(url) {
      ...
      const session = new _YamuxSession(ws);
    ...
    close() {
      ...
      this.streams.clear();
      this.ws.close(1e3, "yamux session closed");
      ...
      while (!this.closed) {
        ...
        switch (type) {
          ...
          case TYPE_GO_AWAY: {
            ...
            this.streams.clear();
            this.ws.close(1e3, "received GoAway");
            return;
          }
        }
      }
    ...
    sendFrameRaw(type, flags, streamID, length, data) {
  ...
  // ../../ts/dist/src/stream.js
  var FRAME_DATA = 0;
  var FRAME_ERROR = 1;
  var FRAME_HEADER_SIZE = 5;
  ...
  // ../../ts/dist/src/transport.js
  async function openStream(session, method) {
    const ys = await session.open();
    const ws = new WSStream(ys);
    await ws.send(new TextEncoder().encode(method));
    return ws;
  }
  async function callUnary(session, method, reqBytes, fromBinary2) {
    const ws = await openStream(session, method);
    await ws.send(reqBytes);
    const data = await ws.recv();
    ws.close();
    return fromBinary2(data);
  }
  var ServerStream = class {
    stream;
    fromBinary;
    done = false;
    constructor(stream, fromBinary2) {
      this.stream = stream;
      this.fromBinary = fromBinary2;
    }
  ...
  var ClientStream = class {
    ...
    toBinary;
    fromBinary;
    constructor(stream, toBinary2, fromBinary2) {
      this.stream = stream;
      this.toBinary = toBinary2;
      this.fromBinary = fromBinary2;
    }
  ...
  // gen/v1/identity_pb.ts
  var file_v1_identity = /* @__PURE__ */ fileDesc("", [file_google_protobuf_timestamp]);
  var CreateUserRequestSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 0);
  var CreateUserResponseSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 1);
  var GetUserRequestSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 2);
  var GetUserResponseSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 3);
  var WatchUsersRequestSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 4);
  var UserEventSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 5);
  var ImportUsersResponseSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 6);
  var SyncUsersRequestSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 7);
  var SyncUsersResponseSchema = /* @__PURE__ */ messageDesc2(file_v1_identity, 8);

  // gen/v1/identity.ws.ts
  var IdentityServiceWSClient = class {
    constructor(session) {
  ...
  // ../k6/stress-harness.ts
  var params = new URLSearchParams(location.search);
  var testName = params.get("test") || "all";
  var iterations = parseInt(params.get("n") || "50", 10);
  function log(msg) {
  ...
  async function runUnary(client) {
  ...
  async function runServerStream(client) {
  ...
  async function runClientStream(client) {
  ...
  async function runConcurrent(client) {
  ...
  async function runRawEchoSession(session) {
  ...
  async function runRawTransformSession(session) {
    const ws = await openStream(session, "raw.transform");
    const payload = new Uint8Array([15, 240, 170, 85]);
    await ws.send(payload);
    const result = await ws.recv();
    if (result[0] !== 240) throw new Error(`XOR failed: ${result[0]}`);
    ws.close();
  }
  async function runRawUploadSession(session) {
    ...
    if (total !== 1e3) throw new Error(`expected 1000, got ${total}`);
  }
  var tests = {
    unary: runUnary,
  ...
  async function main() {
    ...
    log(`stress test: "${testName}" \xD7 ${iterations} iterations`);
...
`);
    ...
    const testKeys = testName === "all" ? allKeys : [testName];
    for (const key of testKeys) {
      ...
      const updateStatus = () => {
        document.getElementById("status").textContent = `Running: ${key} \u2014 ${metrics.completed}/${iterations}`;
      };
      updateStatus();
      ...
      metrics.opsPerSec = latencies.length / totalMs * 1e3;
      metrics.status = "done";
      log(
        `  ${errors === 0 ? "\u2713" : "\u2717"} ${key}: ${latencies.length - errors}/${iterations} ok | avg=${metrics.avgMs.toFixed(1)}ms | p95=${metrics.p95Ms.toFixed(1)}ms | p99=${metrics.p99Ms.toFixed(1)}ms | ops/s=${metrics.opsPerSec.toFixed(1)}` + (errors > 0 ? ` | ${errors} errors` : "")
      );
      (window.__STRESS_METRICS__ ??= []).push(metrics);
    }
    session.close();
    const allMetrics = window.__STRESS_METRICS__;
